...
Username entry — on the main menu screen, prompt the player to enter a username (typed via keyboard in Pygame).
Save result — after game over, automatically save username, score, level_reached, and timestamp to the database.
Leaderboard screen — fetch and display the Top 10 all-time scores inside the game window.
Personal best — fetch the player's best score at game start and display it during gameplay.
Suggested schema:
...
Randomly place a set of wall blocks at each new level.
Guarantee that the blocks do not surround or trap the snake's current position at spawn time.
Collision with an obstacle block → game over (same as border collision).
Food and power-ups must not spawn on obstacle blocks.
